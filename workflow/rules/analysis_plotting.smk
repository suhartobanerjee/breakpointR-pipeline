...
rule bp_counts_violin:
    ...
    output: 
        pdf=expand("{sctrip_dir}/breakpointR-pipeline/{{sample}}/analysis/plots/{{sample}}_bp_counts_violin.pdf", sctrip_dir=config["data_location"]),
        tsv=expand("{sctrip_dir}/breakpointR-pipeline/{{sample}}/analysis/data-tables/{{sample}}_bp_counts_violin.tsv", sctrip_dir=config["data_location"]),
    params: 
        chr_to_exclude=config["chr_to_exclude"] if config["chr_to_exclude"] != "" else '""'
    conda: "../envs/bp_on_counts.yaml"
    shell:
        """
        Rscript workflow/scripts/plotting/bp_counts_violin.R {input.bp_summary} {input.cell_list} {params.chr_to_exclude} {output.tsv} {output.pdf}
        """
...
rule bp_counts_chr_violin:
    input: 
        ...
        chr_len=expand("{sctrip_dir}/breakpointR-pipeline/{{sample}}/species/chr_len.tsv", sctrip_dir=config["data_location"])
    output: 
        pdf=expand("{sctrip_dir}/breakpointR-pipeline/{{sample}}/analysis/plots/{{sample}}_bp_counts_chr_violin.pdf", sctrip_dir=config["data_location"]),
        tsv=expand("{sctrip_dir}/breakpointR-pipeline/{{sample}}/analysis/data-tables/{{sample}}_bp_counts_chr_violin.tsv", sctrip_dir=config["data_location"]),
    params: 
        chr_to_exclude=config["chr_to_exclude"] if config["chr_to_exclude"] != "" else '""',
    ...
    shell:
        """
        Rscript workflow/scripts/plotting/bp_counts_chr_violin.R {input.bp_summary} {params.chr_to_exclude} {input.chr_len} {output.tsv} {output.pdf}
        """
