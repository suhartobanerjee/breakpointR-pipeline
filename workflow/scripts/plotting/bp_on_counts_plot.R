# script to plot breakpoints onto Strand-seq count plots 
# BM Jan 2024
...
breakpoints_counts_plot = function(breakpoints, 
    ...
    if(is.na(output_destination)) stop("ERROR: please specify output destination")
    
    add_overview_plot =F
    
    # taken from mosaicatcher:
    ...
    # filter so same cells in both counts & breakpoints df
    keep_cells = d$cell[d$cell %in% breakpoints$cell]
    d = filter(d, cell %in% keep_cells)
    ...
    if (add_overview_plot == T) {
        ...
        ov_binsizes <- ggplot(unique(d[, .(chrom,start,end)])) +
            ...
            scale_x_log10(labels = format_Mb) +
    ...
    # cairo_pdf(output_destination, width=14, height=10, onefile = T)
    pdf(output_destination, width=14, height=10, onefile = T)
    if (add_overview_plot == T) {
        print(xxx)
    }
    ...
    for (s in unique(d$sample))
    {
        ...
        {
            # s = "RPEWT"
            # ce = unique(d[sample == s,]$cell)[1]
            # 
            # ce_bps = bp_list$keep_breakpoints %>% 
            #     as.data.frame() %>% 
            #     filter(cell == ce) %>% 
            #     mutate(chrom = gsub("chr","",seqnames), 
            #            pos_id = paste(chrom, start,end, sep = "_"))
            # 
            # ce_bps_discard = bp_list$all_breakpoints %>% 
            #     as.data.frame() %>% 
            #     filter(cell == ce) %>% 
            #     mutate(chrom = gsub("chr","",seqnames),
            #            pos_id = paste(chrom, start,end, sep = "_"),
            #            type = ifelse(pos_id %in% ce_bps$pos_id, "keep","discard"))
            # 
            
            ce_bps = breakpoints %>% 
                    as.data.frame() %>%
                    filter(cell == ce) %>%
                    mutate(chrom = factor(gsub("chr","",seqnames)),
                           pos_id = paste(chrom, start,end, sep = "_"))
            ...
            # my_bp  = data.frame(chrom = factor(c(1,1,2),levels = c(1,2)), 
            #                     pos = c(100000000, 200000000, 100000000), 
            #                     type = c("keep","discard","keep"))
            ...
            # add breakpointR annotations
            # ce_bps_discard$chrom = factor(ce_bps_discard$chrom)
            # plt = plt + 
            #     geom_segment(data = ce_bps_discard, 
            #                  aes(xend = breakpoint, x = breakpoint, y=+info_reads_per_bin,
            #                      yend=-info_reads_per_bin, colour = type),
            #                  size = 1) + 
            #     scale_colour_manual(values = c("red","blue"), breaks = c("keep","discard"))
            
            plt = plt +
                geom_segment(data = ce_bps,
                             aes(xend = breakpoint, x = breakpoint, y=+info_reads_per_bin,
            ...
            plt_hist <- ggplot(e.melt) +
                ...
                scale_x_continuous(limits = c(-1, plt_hist_xlim), breaks = pretty_breaks(5), labels = format_Mb) +
...
bp_summary = read.table(args[1],
                            header = T) %>% 
    mutate(cell = word(string = filenames,start = 1,end = 1,sep = fixed(".")),
           breakpoint = plyr::round_any((start + end)/2, 1000000))
